...
pub struct ReportConversion<'info> {
    ...
    #[account(
        mut,
        seeds=[b"campaign_affiliate", campaign_affiliate.campaign.key().as_ref(), campaign_affiliate.affiliate.key().as_ref()],
...
impl<'info> ReportConversion<'info> {
    pub fn report_conversion(&mut self) -> Result<()> {
        ...
        // Transfer commission minus fees to affiliate's wallet.
        transfer_sol(
            self.escrow.to_account_info(),
            self.affiliate.to_account_info(), // TODO: This should go to the affiliate's payout wallet address.
